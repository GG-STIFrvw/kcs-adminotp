...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['decline'])) {
        $new_status = 'declined';
        $stmt = $pdo->prepare("UPDATE appointments SET scheduled_date=?, scheduled_time=?, status=? WHERE id=?");
        $stmt->execute([$new_date, $new_time, $new_status, $id]);
    }
                        ...
                        <form method="POST">
                            ...
                            <td class="actions-cell">
                                ...
                                <?php if ($row['status'] === 'pending'): ?>
                                    ...
                                    <button type="submit" name="decline" class="decline-btn" onclick="return confirm('Decline this appointment?')">Decline</button>
